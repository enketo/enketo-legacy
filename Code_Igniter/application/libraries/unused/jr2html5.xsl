...
-->     
            ...
            <form>
                ...
                <xsl:if test="/h:html/h:head/xf:model/xf:bind[@calculate]">
                    <fieldset id="jr-calculated-items" style="display:none;">
                        ...
                        <xsl:message>WARNING: found calculated items. Support for this will be limited.</xsl:message>
                    </fieldset>
                </xsl:if>
                <xsl:if test="/h:html/h:body//xf:output">
                    <xsl:message>NO SUPPORT: output elements were found (ignored)</xsl:message>
                </xsl:if>
            </form>
            </body>
        </html>
    ...
    <xsl:template match="xf:repeat">
        ...
        </fieldset><xsl:comment>end of repeat <xsl:value-of select="@nodeset" /> </xsl:comment>           
    </xsl:template>
   ...
   -->
    ...
    <xsl:template match="xf:input | xf:upload | xf:item | xf:bind[@jr:preload] | xf:bind[@calculate]">
        ...
        <xsl:if test="not(substring($nodeset, 1, 1) = '/')">
            <xsl:message terminate="yes">ERROR: could not determine absolute path/to/instance/node (terminated)</xsl:message>
        </xsl:if>
        ...
        <xsl:variable name="constraintTokens">
            <xsl:for-each select="str:tokenize($constraint, ' |\(|\)')"><!-- assumes spaces always used.... will fail with eg: &lt;5 -->
            ...
            <input>
                            ...
                            <xsl:attribute name="max">
                                ...
                                <xsl:choose>
                                    <xsl:when test="(string-length($max) &gt; 0) and (number($max) = $max)" >
                                        ...
                                        <!--<xsl:message>value of max attribute set to <xsl:value-of select="$max" /></xsl:message>--><!-- DEBUG -->
                                        <xsl:if test="not(contains($constraint, concat($operator1,'=')))">
                                            <xsl:message>WARNING (node: <xsl:value-of select="$nodeset"/>): max constraint operator1 was changed from <xsl:value-of select="$operator1" /> to <xsl:value-of select="concat($operator1, '=')" /></xsl:message>
                                        </xsl:if>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:message>ERROR (node: <xsl:value-of select="$nodeset"/>): could not process 'max' constraint with value <xsl:value-of select="$max" /> (ignored)</xsl:message>
                                     </xsl:otherwise>
                                </xsl:choose>                           
                            </xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:message>NO SUPPORT (node: <xsl:value-of select="$nodeset"/>): max constraint value is not supported for this data type (ignored)</xsl:message>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:if>
                            ...
                            <xsl:attribute name="min">
                                ...
                                <xsl:choose>
                                    <xsl:when test="(string-length($min) &gt; 0) and (number($min) = $min)" >
                                        ...
                                        <!--<xsl:message>value of min attribute set to <xsl:value-of select="$min" /></xsl:message>--><!-- DEBUG -->
                                        <xsl:if test="not(contains($constraint, concat($operator2,'=')))">
                                            <xsl:message >WARNING (node: <xsl:value-of select="$nodeset"/>): min constraint operator2 was changed from <xsl:value-of select="$operator2" /> to <xsl:value-of select="concat($operator2, '=')" /></xsl:message>
                                        </xsl:if>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:message>ERROR (node: <xsl:value-of select="$nodeset"/>): could not process 'min' constraint with value <xsl:value-of select="$min" /> </xsl:message>
                                    </xsl:otherwise>
                                </xsl:choose>                              
                            </xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:message>NO SUPPORT (node: <xsl:value-of select="$nodeset"/>): min constraint value is not supported for this data type (ignored)</xsl:message>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:if>
                    ...
                    <xsl:attribute name="pattern">
                        <xsl:choose>
                            <xsl:when test="contains($constraint, '&quot;')"> 
                                ...
                                <!--<xsl:message>extracted pattern '<xsl:value-of select="$pattern" />' from '<xsl:value-of select="$constraint" />'</xsl:message>--><!-- DEBUG -->
                            </xsl:when>
                            <xsl:when test='contains($constraint, "&apos;")'>
                                <xsl:variable name="pattern" select='substring-before(substring-after($constraint, "&apos;"), "&apos;")' />
                                <xsl:value-of select="$pattern" />
                            ...
                            <xsl:otherwise>
                                <xsl:message>ERROR (node: <xsl:value-of select="$nodeset"/>): unable to extract regex pattern from '<xsl:value-of select="$constraint" /> (ignored)'</xsl:message>                                
                            </xsl:otherwise>
                        </xsl:choose>        
                    </xsl:attribute>
                </xsl:if>
                ...
                <xsl:if test="string-length($binding/@readonly) &gt;0">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:message>ERROR (node: <xsl:value-of select="$nodeset"/>): unknown value for readonly attribute (ignored)</xsl:message>
                        </xsl:otherwise>                           
                    </xsl:choose>
                </xsl:if>
                    ...
                    <xsl:attribute name="required">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:message>ERROR (node: <xsl:value-of select="$nodeset"/>): unknown 'required' attribute - ignored</xsl:message>
                            </xsl:otherwise>
                        </xsl:choose>        
                    </xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="xf:label | xf:hint">
        ...
        <xsl:choose>
            <xsl:when test="not(string(./@ref)) and string(.)">
                <span lang="">
                    <xsl:if test="local-name()='hint'" >
                        <xsl:attribute name="class">
    ...
    <xsl:template match="xf:trigger">
        <div class="trigger" style="display:none;"><xsl:value-of select="." /></div> 
        <xsl:message>NO SUPPORT: trigger was added in a hidden div but has virtually no support</xsl:message>
    </xsl:template>
    ...
    <xsl:template name="xml_type">
        ...
        <xsl:choose>
            <xsl:when test="string-length($xml_type) &lt; 1" >
                <xsl:value-of select="$error" />

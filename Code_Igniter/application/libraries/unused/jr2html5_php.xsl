...
-->     
    ...
    <xsl:template match="/">
        <!--<xsl:if test="not(function-available('exsl:node-set'))">
            <xsl:message>FATAL ERROR: node-set function is not available in this XSLT processor</xsl:message>
        </xsl:if>
        <xsl:if test="not(function-available('tokenize'))">
            <xsl:message>FATAL ERROR: tokenize function is not available in this XSLT processor</xsl:message>
        </xsl:if>  -->      
        ...
        <html>
            ...
            <form>
                ...
                <xsl:comment>
                ***************************************************************************************************************
                This form was created by transforming a javaRosa-style (X)Form using an XSLT sheet created by Aid Web Solutions
                ***************************************************************************************************************
   ...
   -->
    ...
    <xsl:template match="xf:input | xf:upload | xf:item | xf:bind[@jr:preload] | xf:bind[@calculate]">
        ...
        <xsl:variable name="constraintTokens">
            <xsl:for-each select="tokenize($constraint, ' |\(|\)')"><!-- assumes spaces always used.... will fail with eg: &lt;5 -->
        ...
        <label>
            ...
            <input>
                ...
                <xsl:if test="$html_type = 'file'">
                    <xsl:if test="@mediatype">
                        <xsl:attribute name="accept">
                            ...
                            <xsl:message>WARNING [<xsl:value-of select="$nodeset" />] Until browsers start supporting direct webcam or microphone recording, this is a file input</xsl:message>
                        </xsl:attribute>
                    </xsl:if>
                </xsl:if>
                <!--
                ...
                <!-- ****** MAX ****** -->                
                <xsl:variable name="operator">&lt;</xsl:variable>
                <xsl:if test="contains($constraint, $operator)">                   
                    <xsl:choose>
                        <xsl:when test="$html_type = 'number'">
                            <xsl:attribute name="max">
                                <xsl:variable name="max" select="exsl:node-set($constraintTokens)/token[contains(text(), $operator)]/following-sibling::*/text()"/>
                                <xsl:choose>
                                    <xsl:when test="(string-length($max) &gt; 0) and (number($max) = $max)" >
                                        ...
                                        <xsl:if test="not(contains($constraint, concat($operator,'=')))">
                                            <xsl:message>WARNING [<xsl:value-of select="$nodeset"/>] max constraint operator was changed from <xsl:value-of select="$operator" /> to <xsl:value-of select="concat($operator, '=')" /></xsl:message>
                                        </xsl:if>
                                    </xsl:when>
                                    <xsl:otherwise>
                    ...
                    <xsl:choose>
                        <xsl:when test="$html_type = 'number'">
                            <xsl:attribute name="min">
                                <xsl:variable name="min" select="exsl:node-set($constraintTokens)/token[contains(text(), $operator)]/following-sibling::*/text()"/>
                                <xsl:choose>
                                    <xsl:when test="(string-length($min) &gt; 0) and (number($min) = $min)" >
                                        ...
                                        <xsl:if test="not(contains($constraint, concat($operator,'=')))">
                                            <xsl:message >WARNING [<xsl:value-of select="$nodeset"/>] min constraint operator was changed from <xsl:value-of select="$operator" /> to <xsl:value-of select="concat($operator, '=')" /></xsl:message>
                                        </xsl:if>
                                    </xsl:when>
                                    <xsl:otherwise>

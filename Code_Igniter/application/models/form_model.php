...
class Form_model extends CI_Model {
    ...
    function can_be_loaded_from_cache()
    {
		...
		return ($this->_content_unchanged($this->form_hash_prev) &&  $this->_stylesheets_unchanged($this->xsl_ver_prev));
    }
    ...
    private function _get_formlist()
    {
        ...
        if ($this->formlist_sxe && $this->form_id)
        {
            ...
            foreach ($this->formlist_sxe->xform as $form)
            {
                if ($form->formID == $this->form_id)
                {
                    ...
                    $info['hash'] = (isset($form->hash)) ? (string) $form->hash : NULL;
            		return $info;
                }
            }
    ...
    private function _load_xml($resource)
    {
    	...
    	log_message('debug', 'loading xml from '.$type.': '.$resource);
    	if (isset($type))
    	{
    ...
    private function _content_unchanged($previous_hash = NULL)
    {
        $current_hash = $this->info['hash'];
    ...
    private function _stylesheets_unchanged()
    {
        ...
        return ($xsl_version === $this->xsl_ver_prev);
    }
}
?>